    ...
    const onClickOrder = async () => {
        try {
            ...
            for (let item of itemsCart) {
                await axios.delete(
                    `https://67909a30af8442fd7376e8d8.mockapi.io/todoHexlet/cart/${item.id}`
                );
                await delay(1000);
            }
        } catch (error) {
            ...
            <div className={`${styles.drawer}`}>
                ...
                {items.length > 0 ? (
                    ...
                    <Info
                        title={
                            isOrderCompleted
                                ? `Заказ оформлен №${orderId}`
                                : 'Корзина пустая'
                        }
                        description={
                            isOrderCompleted
                                ? 'Ваш заказ передан на доставку. Пожалуйста ожидайте курьера!'
                                : 'Мы очень надеемся что вы что-то закажите :)'
                        }
                        image={
                            isOrderCompleted
                                ? 'img/order.png'
